...
% Para saber los tranbordos
%linea(N2,L2),linea(N1,L1),member(T,L1),member(T,L2),N1\=N2.


% te dice si dos estaciones estan en la misma linea
% linea(N1,L),member(chabacano,L),member(jamaica,L).

% El pito es un if

% trasbordos de una linea del metro dada un estacion
% linea(N1,L1),linea(N2,L2),member(uami,L1),member(T,L1),member(T,L2),L1\=L2.
...
unTransbordo(E1,E2,[X|Xs]).
unTransbordo(E1,E2,[Y|Xs]):-
				linea(N1,L1),
				member(T)
