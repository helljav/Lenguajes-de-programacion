...
% predicado para hacer a los actores genericos
estrella(M,A,R):- actor(M,A,R);actress(M,A,R).

% Encontrar dos actores o actrices que han trabajado juntos en mas de una película.
movieTogether(A1,A2,M1,M2):- estrella(M1,A1,_),estrella(M1,A2,_),movie(M1,_),estrella(M2,A1,_),estrella(M2,A2,_),movie(M2,_),A1\=A2, M1@<M2.

%Encontrar todos los actores/actrices que hayan trabajado para un mismo director
%Correla: jobDirector(stephen_hillenburg,L).
jobDirector(D,L):-findall(E,(director(M,D),estrella(M,E,R)),L).

%Encontrar el actor o actriz con mas actuaciones en un año.
actuaciones(E):-estrella(M1,E,_),movie(M1,Y1),estrella(M2,E,_),movie(M2,Y2),Y1==Y2.
